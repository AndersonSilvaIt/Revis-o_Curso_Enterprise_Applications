...
namespace NSE.WebApp.MVC.Controllers
{
    public class CarrinhoController : MainController
    {
        ...
        public async Task<IActionResult> AtualizarItemCarrinho(Guid produtoId, int quantidade)
        {
            ...
            if (ResponsePossuiErros(response)) return View("Index", await _carrinhoService.ObterCarrinho());
        ...
        private void ValidarItemCarrinho(ProdutoViewModel produto, int quantidade)
        {
            ...
            if (quantidade > produto.QuantidadeEstoque) 
                AdicionarErroValidacao($"O produto {produto.Nome} possui {quantidade} unidades em estoque, você selecionou {quantidade}");
